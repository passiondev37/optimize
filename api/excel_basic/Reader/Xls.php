...
class Xls extends AbstractReader {
    ...
    public $_defaultFormat = self::SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
     ...
     */
    public $dateFormats = array(0xe => "m/d/Y", 0xf => "M-d-Y", 0x10 => "d-M", 0x11 => "M-Y", 0x12 => "h:i a", 0x13 => "h:i:s a", 0x14 => "H:i", 0x15 => "H:i:s", 0x16 => "d/m/Y H:i", 0x2d => "i:s", 0x2e => "H:i:s", 0x2f => "i:s.S");

    /**
     * Default number formats used by Excel
     */
    public $numberFormats = array(0x1 => "0", 0x2 => "0.00", 0x3 => "#,##0", 0x4 => "#,##0.00", 0x5 => "\$#,##0;(\$#,##0)", 0x6 => "\$#,##0;[Red](\$#,##0)", 0x7 => "\$#,##0.00;(\$#,##0.00)", 0x8 => "\$#,##0.00;[Red](\$#,##0.00)", 0x9 => "0%", 0xa => "0.00%", 0xb => "0.00E+00", 0x25 => "#,##0;(#,##0)", 0x26 => "#,##0;[Red](#,##0)", 0x27 => "#,##0.00;(#,##0.00)", 0x28 => "#,##0.00;[Red](#,##0.00)", 0x29 => "#,##0;(#,##0)", // Not exactly
        0x2a => "\$#,##0;(\$#,##0)", // Not exactly
        0x2b => "#,##0.00;(#,##0.00)", // Not exactly
        0x2c => "\$#,##0.00;(\$#,##0.00)", // Not exactly
        0x30 => "##0.0E+0");
    public $colors = Array(0x00 => "#000000", 0x01 => "#FFFFFF", 0x02 => "#FF0000", 0x03 => "#00FF00", 0x04 => "#0000FF", 0x05 => "#FFFF00", 0x06 => "#FF00FF", 0x07 => "#00FFFF", 0x08 => "#000000", 0x09 => "#FFFFFF", 0x0A => "#FF0000", 0x0B => "#00FF00", 0x0C => "#0000FF", 0x0D => "#FFFF00", 0x0E => "#FF00FF", 0x0F => "#00FFFF", 0x10 => "#800000", 0x11 => "#008000", 0x12 => "#000080", 0x13 => "#808000", 0x14 => "#800080", 0x15 => "#008080", 0x16 => "#C0C0C0", 0x17 => "#808080", 0x18 => "#9999FF", 0x19 => "#993366", 0x1A => "#FFFFCC", 0x1B => "#CCFFFF", 0x1C => "#660066", 0x1D => "#FF8080", 0x1E => "#0066CC", 0x1F => "#CCCCFF", 0x20 => "#000080", 0x21 => "#FF00FF", 0x22 => "#FFFF00", 0x23 => "#00FFFF", 0x24 => "#800080", 0x25 => "#800000", 0x26 => "#008080", 0x27 => "#0000FF", 0x28 => "#00CCFF", 0x29 => "#CCFFFF", 0x2A => "#CCFFCC", 0x2B => "#FFFF99", 0x2C => "#99CCFF", 0x2D => "#FF99CC", 0x2E => "#CC99FF", 0x2F => "#FFCC99", 0x30 => "#3366FF", 0x31 => "#33CCCC", 0x32 => "#99CC00", 0x33 => "#FFCC00", 0x34 => "#FF9900", 0x35 => "#FF6600", 0x36 => "#666699", 0x37 => "#969696", 0x38 => "#003366", 0x39 => "#339966", 0x3A => "#003300", 0x3B => "#333300", 0x3C => "#993300", 0x3D => "#993366", 0x3E => "#333399", 0x3F => "#333333", 0x40 => "#000000", 0x41 => "#FFFFFF",
        0x43 => "#000000", 0x4D => "#000000", 0x4E => "#FFFFFF", 0x4F => "#000000", 0x50 => "#FFFFFF", 0x51 => "#000000",
        0x7FFF => "#000000");
    public $lineStyles = array(0x00 => "", 0x01 => "Thin", 0x02 => "Medium", 0x03 => "Dashed", 0x04 => "Dotted", 0x05 => "Thick", 0x06 => "Double", 0x07 => "Hair", 0x08 => "Medium dashed", 0x09 => "Thin dash-dotted", 0x0A => "Medium dash-dotted", 0x0B => "Thin dash-dot-dotted", 0x0C => "Medium dash-dot-dotted", 0x0D => "Slanted medium dash-dotted");
    ...
    function _parse() {
        ...
        $code = self::v($data, $pos);
        $length = self::v($data, $pos + 2);
        $version = self::v($data, $pos + 4);
        $substreamType = self::v($data, $pos + 6);
            ...
            switch ($code) {
                case self::SPREADSHEET_EXCEL_READER_TYPE_SST :
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = self::v($data, $spos);
                            $conlength = self::v($data, $spos + 2);
                            if ($opcode != 0x3c) {
                                return - 1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = self::v($data, $spos);
                            $spos += 2;
                        }
                ...
                case self::SPREADSHEET_EXCEL_READER_TYPE_FORMAT :
                    $indexCode = self::v($data, $pos + 4);
                    if ($version == self::SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = self::v($data, $pos + 6);
                        if (ord($data [$pos + 8]) == 0) {
                ...
                case self::SPREADSHEET_EXCEL_READER_TYPE_FONT :
                    $height = self::v($data, $pos + 4);
                    $option = self::v($data, $pos + 6);
                    $color = self::v($data, $pos + 8);
                    $weight = self::v($data, $pos + 10);
    ...
    function _getCellDetails($spos, $numValue, $column) {
        ...
        if ($type == 'date') {
            ...
            $dateinfo = Ole::gmgetdate($utcValue);
